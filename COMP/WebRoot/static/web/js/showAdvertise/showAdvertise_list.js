$(function(){
	...
	$.ajax({
		url:ctx+"/club/showAdvertise/navigaitons",
		type:'post',
		async:false,
		success:function(d){
			var navs = d.navs;
	...
	$("body").on("click",".add_partner_btn",function(){
		window.location.href=ctx+"/club/showAdvertise/add";
	});
	...
	$("body").on("click",".confirm_delte",function(){
		...
		$.ajax( {
			type : "POST",
			url : ctx+"/club/showAdvertise/delete/"+$("#showId").val(),
			async : false,
			success : function(data) {
				alert(data.msg)
				if(data.type=="1"){
					window.location.href=ctx+"/club/showAdvertise/initPage";
				}
			}
		});
	});
