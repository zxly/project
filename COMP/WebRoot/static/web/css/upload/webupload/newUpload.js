// 图片上传demo
 var localImgWidth =0,localImgHeight=0 ,mWidth=0,mHeight=0,
				 cWidth=0,cHeight=0,index,preview=0;
...
$(function() {
    ...
    if ( !WebUploader.Uploader.support('flash') && WebUploader.browser.ie ) {
        ...
        if (flashVersion) {
            (function(container) {
                ...
                var swf = ctx+'/webupload/expressInstall.swf';
                // insert flash object
    ...
    uploader.on( 'uploadSuccess', function( file,data) {
    	...
    	if(data){
        	...
        	if(jsonData.success){
        	  if(jsonData.preview){
        		  $(".originalImg").attr("src",jsonData.nginxLink);
        	  }else{
        		  $("#fileId_"+index).val(jsonData.id);
            	  $("#upload_img_"+index).attr("src",jsonData.nginxLink);
        	  }        	  
        	}else{
        	  alert("上传失败");
        	}
